...
#region break
execute as @e[type=armor_stand,tag=crafting_station,tag=placed] at @s unless block ~ ~ ~ dropper[facing=down] run kill @e[type=item,nbt={Item: {id: "minecraft:barrel"}},distance=..1]
execute as @e[type=armor_stand,tag=crafting_station,tag=placed] at @s unless block ~ ~ ~ dropper[facing=down] run kill @e[type=item,nbt={Item: {id: "minecraft:red_stained_glass_pane",tag:{Tags:"locked_slot"}}},distance=..1]
